...
{
	if(!ptr->is_leaf)
		return successor(ptr->children[0]);
...
struct node *balance(struct node *ptr)
{
	...
	if(parent != NULL && parent->numKeys == 0)
	...
	for(int i = 0; i < ptr->numKeys; i++)
	{
		if(ptr->keys[i] == val)
		{
			if(ptr->is_leaf)
			{
				...
				if(ptr->parent != NULL && ptr->numKeys == 0)
					ptr = balance(ptr);
			...
			{
				struct node *child = ptr->children[i+1];
				...
				struct node *succs= successor(child);	
				ptr->keys[i] = succs->keys[0];

				return deleteElement(child, succs->keys[0]);
			}
		}
